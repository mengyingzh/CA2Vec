...
def parse_args():
    parser = ArgumentParser(
        description=('Build a Skip-gram model from the corpus.')
    )
    s_gram = parser.add_argument_group('Skip-gram options')
    s_gram.add_argument('-s', '--vector-size', type=int, default=100,
                         help=('Dimensionality of output word vectors'))
    s_gram.add_argument('-w', '--window-size', type=int, default=2,
                           help=('Number of context words to track to '
                                 'left and right of each word'))
    s_gram.add_argument('-n', '--number-step', type=int, default=5000,
                        help=('Number of training steps'))
    s_gram.add_argument('-c', '--number-cluster', type=int, default=100,
...
def read_data(filename, dict_label):
    ...
    with open(filename, "r") as f:
        ...
        if 0 < count_line < count_line_max:
            for i in range(count_line):
                visit = lines[i].strip('\n')
                logger.info('hhh %s' % visit)
    ...
    # logger76 .info("item_number_per_visit : %s" % item_number_per_visit)
    # logger.info('total visit record number : %s' % count_total_visit)
    # logger.info('total patient number : %s' % patient_num)
    # logger.info('item_number_per_visit: %s', item_number_per_visit[10:])
...
if __name__ == '__main__':
    ...
    corpus_filename = r'.\data\d2bow.txt'
    vocabulary, vocabulary_weight, patient_num, item_count_per_visit = read_data(corpus_filename, dict_label)
    logger.info('reading raw finished...data size : %s' % len(vocabulary))

    # Step 2: Build the dictionary and replace rare words with UNK token.
    logger.info("building dictionary...")
    vocabulary_size = len(id_med_dict)-5
    ...
    batch_size = 512
    embedding_size = arguments.vector_size  # Dimension of the embedding vector.
    logger.info("hhhhhembedding size %s" % embedding_size)
    skip_window = arguments.window_size  # How many words to consider left and right.
    ...
    num_steps = arguments.number_step
    ...
    # for activity_index in range(30,51):
    #         logger.info("%s : %s" % (id_med_dict[activity_index],embedding_list[dictionary[activity_index]]))
    ...
    #     print("patient_icd_16", patient_icd_label_dict.get(16))
    #     print("patient_icd_21", patient_icd_label_dict.get(21))
        ...
        n = arguments.number_cluster
        true_label = patient_icd_label[:n]
        vocab = Counter()
