...
def read_patient_icd(filename_icd):
    with open(filename_icd, "rb") as fname7:
        obj1 = np.load(fname7)
        ...
        for icd_index in range(1, len(visit_info)):
            if len(patient_icd_label) < 5000000:
    ...
    with open(filename, "r") as f:
        ...
        while line:
            if not line.startswith("-1"):
                ...
                if len(item_number_per_visit) == 0:
                    item_number_per_visit.append(len(temp_list))
                else:
                    item_number_per_visit.append(len(temp_list) + item_number_per_visit[-1])
                for _ in temp_list:
                    item_sub.append(_)
            else:
                patient_num += 1
                if len(visit_number_per_patient) == 0:
                    visit_number_per_patient.append(temp_visit_count)
                else:
                    visit_number_per_patient.append(temp_visit_count
                                                    + visit_number_per_patient[-1])
                temp_visit_count = 0
            line = f.readline().strip('\n')
    return item_sub, item_dosage_sub, patient_num, item_number_per_visit, visit_number_per_patient
...
def generate_batch(batch_size):
    global data_index
    global visit_index
    global patient_batch_index
    # assert batch_size % num_skips == 0
    # assert num_skips <= 2 * skip_window
    batch = np.ndarray(shape=(batch_size), dtype=np.int32)
    labels = np.ndarray(shape=(batch_size, 1), dtype=np.int32)
    # logger.info("visit index %s" % visit_index)
    if data_index > window_size_list[visit_index]:
        visit_index += 1

    if visit_index > window_size_list_v[patient_batch_index]:
        patient_batch_index += 1

    if patient_batch_index == 0:
        local_visit_len = window_size_list_v[0]
    else:
        local_visit_len = window_size_list_v[patient_batch_index] - window_size_list_v[patient_batch_index - 1]

    if visit_index == 0:
        visit_item_num_nearest = window_size_list[visit_index]
    else:
        visit_item_num_nearest = window_size_list[visit_index] - window_size_list[visit_index-1]

    skip_window = max(2, visit_item_num_nearest //4)
    while batch_size % skip_window != 0:
        skip_window -= 1
    span = 2 * skip_window + 1  # [ skip_window target skip_window ]
    num_skips = max(2, span-1)  # number_skips: How many times to reuse an input to generate a label.
    ...
    # local_window_len = span - 2

    global_window_index = window_size_list[visit_index]

    buffer.extend(data[data_index:data_index + span])

    data_index += span
    temp = visit_item_num_nearest
    # logger.info(buffer)
    for i in range(batch_size // num_skips):
        ...
        labels[i * num_skips + num_skips - 2, 0] = data[global_window_index]
        labels[i * num_skips + num_skips - 1, 0] = data[global_window_index + 1]
...
if __name__ == '__main__':
    ...
    filename_raw = r'.\data\d2bow.txt'
    filename_icd = r'.\data\d2diag.np'

    patient_icd_label = read_patient_icd(filename_icd)
    ...
    cohort, vocabulary_weight, patient_num, window_size_list, window_size_list_v = read_data(filename_raw)
